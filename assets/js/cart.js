...
function quantityControl(item) {
  ...
  return `<div class="quantity-block">
    <div class="quantity" aria-label="Controle de quantidade">
      <button class="quantity-button" type="button" data-minus="${item.id}" ${atMinimum ? 'disabled' : ''} aria-label="Diminuir quantidade">−</button>
      <input class="quantity-input" data-quantity="${item.id}" type="number" min="1" max="${stock}" value="${quantity}" inputmode="numeric" aria-label="Quantidade de ${escapeHtml(item.product.name)}">
      <button class="quantity-button" type="button" data-plus="${item.id}" ${atMaximum ? 'disabled' : ''} aria-label="Aumentar quantidade">+</button>
    </div>
    <small class="quantity-helper">${stockMessage}</small>
  </div>`;
}
...
function render() {
  ...
  list.innerHTML = items.map(item => `<article class="cart-item">
    ${media(item.product, 'thumb')}
    <div class="cart-item-copy">
      <span class="eyebrow">${escapeHtml(item.product.brand)}</span>
      <h2><a href="/produto/${item.product.id}/">${escapeHtml(item.product.name)}</a></h2>
      <p>${escapeHtml(item.product.volume)}</p>
      <strong>${money.format(Number(item.product.price))}</strong>
    </div>
    ${quantityControl(item)}
    <button class="remove-link" type="button" data-remove="${item.id}">Remover</button>
  </article>`).join('');
...
document.addEventListener('click', event => {
  ...
  if (plus) {
    const input = document.querySelector(`[data-quantity="${plus.dataset.plus}"]`);
    updateCart(plus.dataset.plus, Number(input.value) + 1);
    render();
    return;
  }
  ...
  if (minus) {
    const input = document.querySelector(`[data-quantity="${minus.dataset.minus}"]`);
    updateCart(minus.dataset.minus, Number(input.value) - 1);
    render();
  }
});
