...
function mapOrder(status:string){return status==="processed"?"Pagamento aprovado":status==="processing"?"Pagamento em análise":status==="failed"?"Pagamento recusado":["canceled","expired"].includes(status)?"Cancelado":status==="refunded"?"Reembolsado":"Aguardando pagamento";}
...
async function validSignature(req:Request,dataId:string,secrets:string[]){
  const signature=req.headers.get("x-signature")||"";const requestId=req.headers.get("x-request-id")||"";
  if(!signature||!requestId||!dataId||!secrets.length)return false;
  const parts:Record<string,string>={};for(const part of signature.split(",")){const [k,v]=part.trim().split("=",2);if(k&&v)parts[k]=v;}
  const ts=parts.ts||"";const received=parts.v1||"";if(!ts||!received)return false;
  const numericTs=Number(ts);if(!Number.isFinite(numericTs))return false;
  const tsMs=numericTs>1e12?numericTs:numericTs*1000;if(Math.abs(Date.now()-tsMs)>10*60*1000)return false;
  const manifest=`id:${dataId};request-id:${requestId};ts:${ts};`;
  for(const secret of secrets){if(safeEqual(await hmacHex(secret,manifest),received))return true;}
  return false;
}
...
function methodMatches(orderMethod:string,payment:any){const id=String(payment?.payment_method?.id||payment?.payment_method_id||"").toLowerCase();const type=String(payment?.payment_method?.type||payment?.payment_type_id||"").toLowerCase();return orderMethod==="pix"?(id==="pix"||type==="bank_transfer"):["credit_card","debit_card"].includes(type);}

Deno.serve(async(req:Request)=>{
  if(req.method!=="POST")return json({error:"Método não permitido."},405);
  const contentLength=Number(req.headers.get("content-length")||0);if(contentLength>65536)return json({error:"Solicitação muito grande."},413);
  try{
    ...
    const productionHint=url.searchParams.get("mode")==="production";
    const configuredEnvironment=settings.payment_environment==="production"?"production":"test";
    const environment=productionHint?"production":configuredEnvironment;
    ...
    if(secrets.length&&!await validSignature(req,resourceId,secrets))return json({error:"Assinatura inválida ou expirada."},401);

    if(environment==="production"){
      const {error:verificationError}=await admin.from("store_settings").update({payment_webhook_verified_at:new Date().toISOString()}).eq("id",1);
      if(verificationError)throw verificationError;
    }
    if(productionHint&&configuredEnvironment!=="production")return json({ok:true,readiness:"production_webhook_verified"});

    const liveMode=body.live_mode;
    if(environment==="production"&&liveMode===false)return json({ok:true,ignored:"test_notification_in_production"});
    if(environment==="test"&&liveMode===true)return json({ok:true,ignored:"live_notification_in_test"});

    const testTokens=[Deno.env.get("MERCADO_PAGO_ACCESS_TOKEN_CARD_TEST"),Deno.env.get("MERCADO_PAGO_ACCESS_TOKEN_TEST")];
    const prodTokens=[Deno.env.get("MERCADO_PAGO_ACCESS_TOKEN_CARD_PRODUCTION"),Deno.env.get("MERCADO_PAGO_ACCESS_TOKEN_PRODUCTION")];
    const tokens=(environment==="production"?prodTokens:testTokens).filter((v):v is string=>Boolean(v));
    if(!tokens.length)throw new Error("Credenciais do ambiente não configuradas.");
    await admin.rpc("expire_order_inventory_reservations");

    if(type==="order"||type==="orders"||resourceId.startsWith("ORD")){
      ...
      const{data:order,error:orderError}=await admin.from("orders").select("id,total,payment_method,mercado_pago_order_id,mercado_pago_payment_id,status").eq("id",external).maybeSingle();
      if(orderError)throw orderError;if(!order)return json({ok:true,ignored:"unknown_order"});
      ...
      if(!amountMatches(payment.amount??mp.total_amount,expected)||!methodMatches(order.payment_method,payment))return json({error:"Cobrança divergente do pedido."},409);
      const mpStatus=String(payment.status||mp.status||"action_required");const detail=String(payment.status_detail||mp.status_detail||"waiting_transfer").slice(0,480);
    ...
    const payment=await fetchMP(`/v1/payments/${encodeURIComponent(resourceId)}`,tokens);
    const orderId=String(payment.external_reference||"");if(!/^[0-9a-f-]{36}$/i.test(orderId))return json({ok:true,ignored:"invalid_external_reference"});
    const{data:order,error:orderError}=await admin.from("orders").select("id,total,payment_method,mercado_pago_payment_id,status").eq("id",orderId).maybeSingle();
    if(orderError)throw orderError;if(!order)return json({ok:true,ignored:"unknown_order"});
    ...
    const expected=environment==="test"&&order.payment_method==="pix"?50:Number(order.total);
    if(!amountMatches(payment.transaction_amount??payment.amount,expected)||!methodMatches(order.payment_method,payment))return json({error:"Pagamento divergente do pedido."},409);
    const mpStatus=String(payment.status||"");const detail=String(payment.status_detail||"").slice(0,480);
