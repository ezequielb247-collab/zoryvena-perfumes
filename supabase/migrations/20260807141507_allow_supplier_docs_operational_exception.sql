alter table public.store_settings
  add column if not exists supplier_docs_unavailable_acknowledged_at timestamptz;

comment on column public.store_settings.supplier_docs_unavailable_acknowledged_at is
  'Operational acknowledgement that supplier provenance documents are currently unavailable. This is not proof of provenance and must not be treated as supplier_docs_verified.';

update public.store_settings
   set supplier_docs_unavailable_acknowledged_at = coalesce(supplier_docs_unavailable_acknowledged_at, now())
 where id = 1
   and supplier_docs_verified = false;
...
declare
  ...
  v_supplier_requirement_met boolean;
...
begin
  ...
  v_supplier_requirement_met := v_supplier_docs_verified
    or v_settings.supplier_docs_unavailable_acknowledged_at is not null;

  if v_payment_environment = 'production' and not v_supplier_requirement_met then
    raise exception 'Produção exige procedência confirmada ou exceção operacional registrada para documentos indisponíveis.';
  end if;
  ...
  if v_launch_status in ('soft_launch','live') and (
       v_payment_environment <> 'production' or not v_supplier_requirement_met
     ) then
    raise exception 'Lançamento bloqueado até pagamentos em produção e requisito de fornecedor estarem registrados.';
  end if;
